# Joshua Lang - Email Evidence Report

**Year group:** Year 10 (ks4-joshua-lang)  
**School (tracker):** All Saints  
**Search date:** 2026-06-11  
**Summary:** All Saints live place; 76% att; Rapid Response accepted; referral placeholder on disk

## Sources searched

- HxStore.hxd (`data/emails-extracted/joshua-lang.txt`)
- `general-ks4-terms.txt`, `outlook-search-report.txt`

## Email evidence

### Thread 1: See below regarding Joshua Lang

| Field | Detail |
|-------|--------|
| Subject (body lead-in) | `See below regarding Joshua Lang.` |
| Source | HxStore audit (`joshua-lang.txt`) |

**Quotes / snippets:**

- `Hi both, See below regarding Joshua Lang. Would it be possible to arrange a review meet for this Thursday please? 10am ok?`

**Topics:** review meeting, school liaison

---

### Thread 2: Joshua Lang and Daniel Jolaoso

| Field | Detail |
|-------|--------|
| Subject | `Joshua Lang and Daniel Jolaoso` |
| Date | 15 May 2026 at 09:02:32 BST |
| Sender | Mrs P Martin &lt;pm @AllSaintsSchool.co.uk&gt; |
| Recipients | Admin Support; Mr S Lucraft; Ms A Armstrong; Miss C Spicer |
| Source | HxStore cache |

**Quotes / snippets:**

- `Good morc` (truncated opening)
- Related: Daniel Jolaoso EHCP emergency review referenced elsewhere in cache (`Casked us to arrange an Emergency Annual Review of Daniel Jolaoso's EHCP`)

**Topics:** school liaison (All Saints), linked case with Daniel Jolaoso

---

### Thread 3: Re: Joshua Lang - KS4 100%

| Field | Detail |
|-------|--------|
| Subject | `Re: Joshua Lang - KS4 100%` |
| Date | 22 April 2026 14:50 |
| Sender | Mr S Lucraft @ allsaintsschool.co.uk |
| Recipients | Eugene Dwaah; Admin Support |
| Source | HxStore cache, `outlook-search-report.txt` |

**Topics:** KS4 100% placement

---

### Thread 4: RAPID RESPONSE REFERRALS (batch) + Lesson Access

| Field | Detail |
|-------|--------|
| Batch date | 9 June 2026 |
| Source | HxStore cache |

**Quotes / snippets:**

- Table: `Joshua Lang` - **All Saints** (live placement)
- Lesson Access ticket 2026-04-14: `Details: Calum MiC Joshua Lang, Y Tyler Redgrave Dennis Ungureanu2 9s are all un s via lark ndar. It lets m log in but missing from Please can we ge? pm added`

**Topics:** rapid response, Purple Ruler / lesson access, portal login

---

### Audit excerpts

- `See below regarding Joshua Lang.` (excerpt 1)
- `Joshua Lang and Daniel Jolaoso` (excerpt 2)

## Outlook Web (full extract)

**Extraction date:** 2026-06-11  
**Method:** cursor-ide-browser MCP; search "Joshua Lang"; opened "Joshua Lang - KS4 100%" (expanded thread)  
**Emails opened:** 1 (multi-message thread)

### Email: Joshua Lang - KS4 100%

| Field | Detail |
|-------|--------|
| Subject | `Joshua Lang - KS4 100%` |
| School | All Saints Catholic School |
| Participants | Bal Gill, Simon Lucraft (All Saints), Lloyd Dwaah, Eugene Dwaah |

**Bal Gill, Tue 03/06/2026 11:15**

- `Rapid Response referral for Joshua Lang`
- `Joshua is currently not attending school`

**Simon Lucraft, Tue 03/06/2026 14:22**

- `Joshua has been absent from school for a significant period`
- `we are supportive of a placement at 100% Sports and Education Centre`

**Lloyd Dwaah, Wed 04/06/2026 07:45**

- `Joshua is already with us.`

**Action items:** Rapid Response referral; school liaison All Saints; placement already active at 100%

---

## Progress & gaps summary

**Data as of:** 2026-06-11 (cached Google Sheets CSV exports)

### Where they are (tracker)

| Source | Field | Value |
|--------|-------|-------|
| Master list | BFL | Doing Well / OK |
| Master list | Intervention | Mentoring |
| Master list | Start date | 03/05/2026 |
| Master list | Reviews | 6w 14/06/2026; 12w 26/07/2026; 24w 18/10/2026; 32w 13/12/2026 |
| Weekly concerns | Att% | 76.09% |
| Weekly concerns | Action | Parent school review scheduled; intervention plan |
| Y10 provision | All courses | Started |
| King's Trust | All | Completed |
| Email cohort list | All Saints live place | `student-updates-10-june-2026-thread.txt` |

### Identified gaps

| Gap | Evidence |
|-----|----------|
| 6-week review due imminently | 14/06/2026 master list |
| Parent school review and intervention plan pending | Weekly concerns |
| No sheet evidence for ILP, risk assessment, student profile | Report gaps |

**Evidence sources:** Google Sheets CSVs, Outlook cohort thread


## Gaps

- Rapid Response referral PDF on disk is image-based; companion `.txt` is placeholder only (see below)
- No local school reports, profile, HCA, contract, risk assessment, or ILP found
- No email evidence found for: mother's Easter holiday contact (truncated fragment only)

## Rapid Response referral (local placeholder txt)

**Source:** `Rapid Response Referral - Joshua Lang.txt`

| Field | Value |
|-------|-------|
| School | All Saints |
| Year group | Year 10 |
| Cohort | KS4 |
| Start date (100% SEC) | 03/03/2026 |

Note in file: placeholder for linking from KS4 summary page; full referral details not yet added.

**Cross-reference:** Outlook KS4 100% thread confirms Joshua already with provision (Lloyd, 04/06/2026); Bal Gill Rapid Response referral 03/06/2026; school supportive of placement.

## Name variations searched

- Joshua Lang, Joshua Lang7 (list truncation)
## Bromcom data

**Extraction date:** 2026-06-11  
**Source:** Bromcom MIS (`https://cloudmis.bromcom.com`), signed-in session  
**Profile URL:** `https://cloudmis.bromcom.com/Nucleus/UI/Areas/Students/StudentDetails.aspx?StudentIDs=125#Profile`  
**Record located:** Yes (Bromcom Student ID 125)

### Enrolment

| Field | Value |
|-------|-------|
| Legal name | Joshua Lang |
| Sex | Male |
| DOB | 25/05/2011 |
| Age | 15y |
| Year group | Y10 |
| Tutor group | KS4 Reg |
| Tutor name | A Dwaah |
| Admission number | 000030 |
| Admission date (list view) | 05/03/2026 |
| EAL | No |

### Attendance highlights

| Metric | Value |
|--------|-------|
| Sessions absent | 14/108 |
| Absent percentage | 12.96% |

Recent AM codes: N on 11 and 10 Jun (communications to All Saints contacts).

### Contacts

| Contact | Relationship | Priority | Email |
|---------|--------------|----------|-------|
| Miss Patricia Martin #87 | Other Non-Family Contact | 1 | pmartin@AllSaintsSchool.co.uk |
| Mr All Saints Attendance Contacts #48 | Other Non-Family Contact | 2 | mincedal@allsaintsschool.co.uk |
| Miss C Spicer #126 | Other Non-Family Contact | 3 | Cspicer@allsaintsschool.co.uk |

Support events: no data on profile. Emergency contacts: none.

### Data quality flags

3 issues: Parental Responsibility, Contact 1 Phone, Contact 2 Phone.

## Student intelligence report

**Source:** WhatsApp Chat - 100% Staff Group.zip (`_chat.txt`)

### Comments and observations

- 05/06/2026: Incident report by Lorelle Gavillet.
- During a King's Trust Teamwork session Joshua was accidentally struck on the nose by a football.
- Joshua experienced a nosebleed.
- First aid was provided.
- Parent contacted.
- Incident deemed accidental.
- Student who kicked the ball apologised.
- Joshua later reported feeling okay and not in pain.

### Stresses and concerns

- Minor injury incident.

### Strengths and positives

- Positive participation in group activity.
- No ongoing concerns reported following incident.

### Actions / interventions mentioned

- First aid.
- Parent contact.
- Incident report completed.

### Gaps / missing information

- No behavioural concerns.
- No safeguarding concerns.
- No academic information.
