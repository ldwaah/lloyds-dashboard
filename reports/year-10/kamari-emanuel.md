# Kamari Emanuel - Email Evidence Report (optional - Y10-related in emails)

**Year group (tracker):** Unknown (ks4-kamari-emanuel)  
**School (tracker):** Barking Abbey  
**Search date:** 2026-06-11  
**Summary:** 2+ threads identified, 2 HxStore audit excerpts. Included because named in Rapid Response batch and induction emails.

## Sources searched

- HxStore.hxd (`data/emails-extracted/kamari.txt`)
- `general-ks4-terms.txt`, ks4-students.js evidenceWording (Outlook Web Jun 2026 - partial)

## Email evidence

### Thread 1: Kamari Emanuel - induction arrangements

| Field | Detail |
|-------|--------|
| Subject | `Kamari Emanuel - contact school cc Bal for induction[` / `Kamari Emanuel's induction arrangements` |
| Source | HxStore audit (`kamari.txt`) |

**Quotes / snippets:**

- `%Kamari Emanuel - contact school cc Bal for induction[`
- `Kamari Emanuel's induction arrangements`

**Topics:** induction, Barking Abbey, school liaison (cc Bal)

---

### Thread 2: Lloyd reply re Barking Abbey (related)

| Field | Detail |
|-------|--------|
| Source | HxStore cache |

**Quotes / snippets:**

- `Hi, Noted. I can contact Barking Abbey and cc Bal regard Kamari Emanuel s induction arrangements`

**Topics:** induction, action item for Lloyd

---

### Thread 3: RAPID RESPONSE REFERRALS (batch email)

| Field | Detail |
|-------|--------|
| Date | 9 June 2026 at 11:36:48 BST |
| Source | `general-ks4-terms.txt` |

**Quotes / snippets:**

- Kamari Emanuel listed in batch table with Barking Abbey KS3/KS4 cases
- Referral detail fragment: `1. Kamari Emanuel ACCEPT` - `Suitable for 100%SEC` - Key Issues: `Anger management.` - `Two physical altercations since returning to school`

**Topics:** rapid response referral, suitability for 100%, behaviour concerns

---

### Student updates (referenced in ks4-students.js, not fully opened in this session)

- Email 10 Jun 2026 (Lloyd Dwaah, Student updates, sent): Lloyd to contact Barking Abbey and cc Bal regarding Kamari Emanuel's induction arrangements
- Admin Support fragment: `Who is this call me Kamari Emanuel` (per seed evidence - subject not in audit file)

**Source note:** Student updates thread visible in Outlook inbox search box history; full body not extracted to audit file in this run.

## Outlook Web (full extract)

**Extraction date:** 2026-06-11  
**Method:** cursor-ide-browser MCP; search `Kamari Emanuel` returned 0 message options; HxStore + Student updates fragments  
**Emails opened:** 0 (name search empty; cohort references only)

### Email: Kamari Emanuel induction (HxStore)

| Field | Detail |
|-------|--------|
| Subject | `Kamari Emanuel - contact school cc Bal for induction[` / `Kamari Emanuel's induction arrangements` |
| School | Barking Abbey |

**HxStore fragments:**

- `%Kamari Emanuel - contact school cc Bal for induction[`
- `Kamari Emanuel's induction arrangements`

### Email: Lloyd Dwaah reply (Student updates, HxStore)

- `I can contact Barking Abbey and cc Bal regard Kamari Emanuel s induction arrangements`

### Email: RAPID RESPONSE REFERRALS (HxStore fragment)

- `1. Kamari Emanuel ACCEPT` - `Suitable for 100%SEC` - `Anger management.` - `Two physical altercations since returning to school`

**Action items:** contact Barking Abbey for induction, cc Bal

**Gap:** dedicated Kamari name search returned no results in Outlook Web this session

---

## Gaps

- No email evidence found for: confirmed Year 10 year group
- No email evidence found for: placement type, start date, funding type
- No email evidence found for: risk assessment, ILP, student profile
- Full Student updates sent-item body not in `data/emails-extracted/`
- Year group remains Unknown in tracker

## Name variations searched

- Kamari Emanuel, Kamari
